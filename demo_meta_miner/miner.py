...
def demo(url,auth):
    ...
    table_data = {}
    distributions = []
    dataset = utils.create_req(model="dataset",name= "testDatabase",app="aristotle_dse")
    ...
    for table in metadata.tables.keys():
        ...
        for columns in metadata.tables[table].c:
            value_domain = utils.create_req(model="valuedomain",name= columns.type.__repr__(),app="aristotle_mdr")
            extra_information_dataelement = {"valueDomain": value_domain}
            data_element = utils.create_req(model="dataelement",name= columns.name.__repr__(),app="aristotle_mdr", other_field_data=extra_information_dataelement)
            extra_information_distribution['data_elements'].append({'data_element' : data_element, "logical_path": table+"."+columns.name.__repr__()})
        distribution = utils.create_req(model="distribution",name= table,app="aristotle_dse",other_field_data=extra_information_distribution)
        distributions.append(distribution)
    utils.save_req_file(distributions)
    conn.close()
